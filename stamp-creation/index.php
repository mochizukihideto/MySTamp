...
// すべてのスタンプを取得（一時的な措置）
$sql = "SELECT * FROM stamps ORDER BY created_at DESC";
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->get_result();
$saved_stamps = $result->fetch_all(MYSQLI_ASSOC);
...
<head>
    ...
    <title>習い事のオリジナルスタンプ作成 - <?php echo htmlspecialchars($user['name']); ?>さんのページ</title>
    <link rel="stylesheet" type="text/css" href="styles.css">
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Dancing+Script&family=Fredoka+One&family=Indie+Flower&family=Pacifico&family=Permanent+Marker&family=Roboto&family=Noto+Sans+JP&display=swap" rel="stylesheet">
    <style>
        .font-preview {
            display: flex;
            flex-wrap: wrap;
...
<body>
    <div class="container">
        <header>
            <h1>ようこそ、<?php echo htmlspecialchars($user['name']); ?>さん！</h1>
            <nav>
                <ul>
        ...
        <div id="stampSelection">
            <h2>スタンプ一覧</h2>
            <div id="savedStamps">
                <?php if (!empty($saved_stamps)): ?>
                    <?php foreach ($saved_stamps as $index => $stamp): ?>
                        <div class="saved-stamp <?php echo ($index === 0 && $new_stamp_created) ? 'new-stamp' : ''; ?>" data-stamp-id="<?php echo htmlspecialchars($stamp['id']); ?>">
                            <img src="/lesson-management-system<?php echo htmlspecialchars($stamp['image_path']); ?>" alt="Saved Stamp">
                            <p><?php echo htmlspecialchars($stamp['hobby']); ?></p>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p>保存されたスタンプはありません。新しいスタンプを作成してみましょう！</p>
                <?php endif; ?>
            </div>
        </div>
